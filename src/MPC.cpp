...
// TODO: Set the timestep length and duration. Horizon T = N * dt
size_t N = 10;
double dt = 0.15;

// This value of Lf assumes the model presented in the classroom is used.
//
...
// state [x, y, psi, v, cte, epsi]    actuators [delta, a] ...where delta is the steering angle and a is acceleration/brake
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    // TODO: implement MPC
    // `fg` a vector of the cost and constraints, 
    ...
    // Initial constraints
    //
    // We add 1 to each of the starting indices due to cost being located at fg[0]
...
//      AD<double> derivative = 3*coeffs[3]*x0*x0 + 2*coeffs[2]*x0 + coeffs[1];
      ...
      //
      // x1 = (x0 + v0 * cos(psi0) * dt) becomes
      // x1 - (x0 + v0 * cos(psi0) * dt) = 0
      //
      // NOTE: The use of `AD<double>` and use of `CppAD`!
      // CppAD can compute derivatives and pass them to the solver.
      ...
      // from equation: cte[i+1] = f(x[i]) - y[i] + v[i] * sin(epsi[i]) * dt
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Acceleration/decceleration upper and lower limits [-1, 1].
